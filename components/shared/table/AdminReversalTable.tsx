...
import { adminReversalColumns } from "./table-columns";
...
const AdminReversalTable = ({ reversals }: Props) => {
  const [globalFilter, setGlobalFilter] = useState<string>("");

  const table = useReactTable({
    data: reversals,
    columns: adminReversalColumns,
    filterFns: {
      fuzzy: fuzzyFilter,
    },
    state: {
    ...
    <>
      {reversals.length === 0 ? (
        <div className="flex h-screen w-full flex-col items-center justify-center gap-7">
          <h2 className="text-[1.125rem] font-[600]">No Reversal Request</h2>
        </div>
      ) : (
        <div className="">
          <div className="space-y-4 rounded-lg bg-light-900 py-4">
            <h2 className="px-2 font-medium">Reversals</h2>
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <td key={cell.id} className="dist-table-style">
                        <Link
                          href={`/admin/disputes/reversals/${row.original.id}`}
                        >
                          {flexRender(
...
export default AdminReversalTable;
