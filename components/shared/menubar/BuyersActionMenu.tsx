...
const actionDetails = {
  suspend: {
    ...
    bodyText:
      "Are you sure you want to suspend this account? This Buyer will be unable to make any transaction if you suspend the account",
  ...
  activate: {
    ...
    bodyText:
      "Are you sure you want to activate this account? This Buyer will be able to make any transaction if you activate the account",
  ...
  delete: {
    ...
    bodyText:
      "Are you sure you want to delete this account? This Buyer will be removed from DieselNg. Note: Verify the buyer doesn't have an active order before deleting.",
    actionBtnText: "Yes, delete",
  },
};

const BuyersActionMenu = ({ buyer }: { buyer: User }) => {
  ...
  const handleAction = async () => {
    if (selectedAction.action === "delete") {
      startTransition(() => {
        adminDeleteUser(buyer.id, pathname).then((data) => {
          if (data?.error) {
      ...
      startTransition(() => {
        adminSuspendUser(buyer.id, selectedAction.action, pathname).then(
          (data) => {
            if (data?.error) {
              toast.error(data.error);
            }
...
export default BuyersActionMenu;
