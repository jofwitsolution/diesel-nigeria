...
import { BuyerBusinessInfoSchema } from "@/lib/validations";
import {
  Form,
  FormControl,
  FormField,
...
import { buyerUpdateBusinessInfo } from "@/lib/actions/buyer.action";
...
const BuyerBusinessInfo = ({ user }: Props) => {
  const pathname = usePathname();
  const [error, setError] = useState<string | undefined>("");
  const [isPending, startTransition] = useTransition();
  const [imageData, setImageData] = useState<string | ArrayBuffer | null>("");

  const form = useForm<z.infer<typeof BuyerBusinessInfoSchema>>({
    resolver: zodResolver(BuyerBusinessInfoSchema),
    defaultValues: {
  ...
  const onSubmit = async (values: z.infer<typeof BuyerBusinessInfoSchema>) => {
    setError("");

    startTransition(() => {
      buyerUpdateBusinessInfo(values, imageData, pathname).then((data) => {
        setError(data.error);
...
export default BuyerBusinessInfo;
